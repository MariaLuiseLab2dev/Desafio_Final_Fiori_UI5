# Portal de Consulta de Saldo — Documentação do Projeto
...
- **Ordem das regiões** em `totalPerRegion` é uma suposição (alfabética) — não documentada no backend, vale confirmar.
- **"Valor por Grupo Comprador"** substitui o pedido original de "status por tipo de despesa" — o backend não tem uma função por Classificação Contábil (Consumo/Imobilizado/Projeto/Serviços), só por Grupo Comprador.
- **CSS do Dashboard** (`webapp/css/style.css`) tem várias sobrescritas pontuais em classes internas do UI5 (`.sapSuiteRMC*`, `.sapMFlexItem`) necessárias pra centralizar/redimensionar os gauges — documentado inline no próprio CSS via nome de classe descritivo.
- **Ano fixo em 2025** no Select de mês porque é o único ano com dados reais confirmados no ambiente de testes; ajustar quando houver dados de outros anos.
